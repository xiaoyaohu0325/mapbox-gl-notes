...
class FillTileApp {
  ...
  updateApps() {
    ...
    if (this.layerIndex) {
      ...
      const app = new DrawLayerApp(this.gl, this.vertexShaderSource, this.fragmentShaderSource);
      app.layer = this.vectorTile.layers['water'];
      this.drawApps.push(app);
      // });
    }
  }
...
class DrawLayerApp extends WebGLBase {
  ...
  parseVectorLayer() {
      ...
      for (const polygon of polygons) {
        ...
        for (const ring of polygon) {
          ...
          for (let i = 0; i < ring.length; i++) {
            ring[i].x = ring[i].x > 0 ? ring[i].x : 0;
            ring[i].y = ring[i].y > 0 ? ring[i].y : 0;
    ...
    this.elementArray = triangleElements;
  }
  ...
  drawScene() {
    ...
    if (this.styleLayer) {
      this.gl.uniform4fv(this.colorUniformLoc, this.styleLayer.getPaintValue('fill-color'));
    } else {
    ...
    this.gl.bufferData(this.gl.ARRAY_BUFFER, new Uint16Array(this.vertexArray), this.gl.STATIC_DRAW);
    ...
    this.gl.bufferData(this.gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(this.elementArray), this.gl.STATIC_DRAW);
    this.gl.drawElements(this.gl.TRIANGLES, this.elementArray.length, this.gl.UNSIGNED_SHORT, 0);
  }
}
