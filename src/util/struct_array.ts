...
export enum ViewType {
  Int8 = 0, Uint8, Uint8Clamped, Int16, Uint16, Int32, Uint32, Float32, Float64
}
 ...
 *  members: [
 *      { type: ViewType.Int16, name: 'x' },
 *      { type: ViewType.Int16, name: 'y' }
 *  ]});
 *
 ...
 * @private
 */
export function createStructArrayType(options: {
  members: Array<StructMember>,
  alignment?: number }) {
...
function createGetter(member, c): () => any {
  return <() => any>new Function(`return ${createMemberComponentString(member, c)};`);
}

function createSetter(member, c): (x: any) => void {
  return <(x: any) => void>new Function('x', `${createMemberComponentString(member, c)} = x;`);
}
